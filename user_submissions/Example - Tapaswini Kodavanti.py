...
class Example(Player):
    ...
    def update(self) -> None:
        # get payoff
        if len(self.self_history) > 0 and len(self.other_history) > 0:
            self.total_payoff += PAYOFF[self.self_history[-1].value][self.other_history[-1].value]
        self.dummy_string += 'a'
