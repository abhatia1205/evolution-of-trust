...
class Example(Player):
    ...
    def act(self) -> Action:
        ...
        if len(self.other_history) > 3 and Action.COOPERATE not in self.other_history:
            return Action.CHEAT
        elif self.other_history[-1] == Action.CHEAT: 
            return Action.CHEAT if random.random() < 0.5 else Action.COOPERATE
        return Action.COOPERATE # If the other player cooperated in the last round, cooperate
